...
**Important notes:**
* This example show just one possibility of implementing a training and a deployment pipeline, there are certainly other ways to achieve the same result with proper Pachyderm use.
* This example is based on a Determined experiment using the **PyTorch Trial API**, some additional changes will be required if your starting Determined experiment is based on CoreAPI or TFKeras Trial API. For example, if you used CoreAPI, you won't be able to load your checkpoint using _load_trial_from_checkpoint_path_ as currently done in **deploy.py**, and if your model hasn't been trained using PyTorch, significant changes will have to be made in **deploy.py**.
...
* Depending on the changes you want to make to this example, the following may also be required:
  * A GitHub account, to fork this example and apply changes to the experiment files.
...
## Step 1: Adapting your experiment files
...
* In **model_def.py**:
  ...
  * In \_\__init___, check if we intend to train the model (requiring downloading data from pachyderm repository, and building the training and validation sets) or not. Don't hesitate to compare both versions of model_def if this is unclear.
...
In this example, we simply changed the _get_train_and_validation_datasets_ from **data.py**, to concatenate csv files into a single pandas DataFrame.
...
* Under _transform_:
  ...
  * Command in _stdin_ is the command that will be run when the pipeline is triggered. Make sure to change all the relevant options, in particular:
    * _--git-url_ if you plan to make any change on the experiment code
    * _--sub-dir_ if the file structure of your git repository is different as this one
...
* We overwrote the following methods from the default _BaseHandler_:
  ...
  * _postprocess_, although that was not necessary? -- CHECK IF THIS CAN BE REMOVED
...
* Finally, in **deploy.py**, in the _create_mar_file_ function, you may want to change the name of the python file where the handler is defined, and well as specifying extra files, if your handler file relies on them.
...
* Under transform:
  ...
  * Command in _stdin_ is the command that will be run when the pipeline is triggered. Make sure to change all the relevant options, in particular:
    ...
    * _--service-account-name_, which is the name of the Service Account for Pachyderm access if you aren't deploying in the cloud
    * _--tolerations_, --resource-requests and --resource-limits, to specify resources to be used by your deployment
    * If you are deploying in the cloud, make sure to check the full list of arguments in **common.py**
  * If you have already created the Kubernetes pipeline secrets, you shouldn't change anything under _secrets_
* Under pod_patch? TO COMPLETE
