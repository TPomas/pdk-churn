...
This example starts from files defining a Determined experiment independent any from Pachyderm integration. It will go through the steps that need to be followed to integrate this experiment to a Pachyderm training pipeline and then to deploy the resulting model with KServe.
...
**Important notes:**
* This is just one implementation example of a training and a deployment pipeline in Pachyderm using a Determined experiment, similar results can be achieved with  with other proper use of Pachyderm.
* This example is based on a Determined experiment using the **PyTorch Trial API**, some additional changes will be required if the starting Determined experiment is based on CoreAPI or TFKeras Trial API. For example, when using CoreAPI, checkpoints cannot be loaded using _load_trial_from_checkpoint_path_ as currently done in **deploy.py**, and if the model hasn't been trained using PyTorch, other significant changes will have to be made in **deploy.py**.

# Prerequisites

* Pachyderm/MLDM + Determined/MLDE + KServe environment properly set up and tested.
...
Be sure to check the difference between the original, base Determined experiment files under **base_experiment** and their version adapted to PDK, under **PDK_implementation/experiment**.
